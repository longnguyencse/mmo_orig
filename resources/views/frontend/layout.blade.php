...
<body class="home">
    ...
    <div id="wrapper">
        ...
        <header>
            ...
            <div class="logo">
                <a href="index-2.html"><img src="{{ 'frontend/img/logo.png' }}" alt=""></a>
        ...
        <div class="cn_wrapper">
            ...
            <div id="cn_preview" class="cn_preview">
             <?php 
            	$hotnews = DB::table("tbl_news")->where("c_hotnews","=","1")->orderBy("pk_news_id","desc")->limit(4)->get();
            	$stt=0;
             ?>

             @foreach($hotnews as $rows_hotnews)
             <?php $stt++; ?>
                <!-- first hot news -->
                <div class="cn_content bg-{{ $stt }}"  style="{{ $stt==1?"top:0px;":""  }} background: url('{{ asset('upload/news/'.$rows_hotnews->c_img) }}') no-repeat center #ffffff;background-size: 100% 100%;">
                    <div class="caption">
                        <h3><a href="{{ url('news/detail/'.$rows_hotnews->pk_news_id) }}">{{ $rows_hotnews->c_name }}</a></h3>
                        <p>{!! $rows_hotnews->c_description !!}</p>
                    </div>
                </div>
                <!-- end first hot news -->
                @endforeach
            ...
            <div id="cn_list" class="cn_list">
                ...
                @foreach($hotnews as $list_hotnews)
                    <!-- list hot news -->
                    <div class="cn_item">
                        <div class="left-box">
                            <img src="{{ asset('upload/news/'.$list_hotnews->c_img) }}" alt="">
                        </div>
                        <div class="right-box">
                            <h4>{{ $list_hotnews->c_name }}</h4>
                        </div>
                        <div class="clear"></div>
                    </div>
                    <!-- end list hot news -->
                       ...
                       <div class="widget">
                            ...
                            <?php 
                             ...
                             @foreach($category as $rows)
                                <li><a class="photo" href="{{ url('news/category/'.$rows->pk_category_news_id) }}">{{ $rows->c_name }}</a></li>
                             @endforeach   
                            </ul>
                            <div class="clear"></div>
                        </div>
                        ...
                        <div class="widget">
                           ...
                           <?php 
                           	$adv = DB::table("tbl_adv")->orderBy("pk_adv_id","desc")->get();
                            ?>
                            @foreach($adv as $rows_adv)
                            <a href="{{ $rows_adv->c_url }}" target="_blank">
                            <img class="ads" src="{{ asset('upload/adv/'.$rows_adv->c_img) }}" alt="">
                            </a>
                            @endforeach
                            <div class="clear"></div>
                        </div>
