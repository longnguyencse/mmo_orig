...
class fixloi extends Command
{
    ...
    $user=DB::table('users')->where('status','=',1)->where('id','=',3)->get(); // get user active    
...
function removeAllFile($dir){
    ...
    $q=(count(glob("public/download/*")) === 0) ? 0 : 1;
        if (0==0){
        foreach ($user as $user_rows) {
          $info=DB::table('info')->where('fk_users_id','=',$user_rows->id)->first();
            if ($info->api != '') { //neu ton tai api
                ...
                $reupchannel = DB::table('reupchannel')->where('pk_reupchannel_id','=',2)->get(); //get channel cua user active
                foreach ($reupchannel as $reupchannel_rows) {
                        ...
                        if ($client->getAccessToken()) { //neu ton tai token
                            try {
                                ...
                                // *******down va xu li video********
                                    ...
                                    $downvideo="$youtubedl -o 'public/download/$namevideo.%(ext)s' https://www.youtube.com/watch?v=$reupvideo_rows->linkreup";
                                        ...
                                        foreach ($arr as $item) {
                                            ...
                                            if ($i==1) {
                                                ...
                                                if($errorcode==403 || !isset($type) || $errorcode==0){
                                                    ...
                                                    $file=glob("public/download/$namevideo.*");
                                                    if(count($file)==0){
                                    ...
                                    if(strpos($duration, 'H')>0){
                                        ...
                                        if($catdau>60){
                                            $mt=floor($catdau/60);
                                            $st=$catdau-$mt*60;
                                            $cmd = "$ffmpeg -y -ss 00:$mt:$st -i public/download/$file_reup -to $hh:$mm:$ss -codec copy public/download/ok$file_reup";
                                        } else{
                                            $cmd = "$ffmpeg -y -ss 00:00:$catdau -i public/download/$file_reup -to $hh:$mm:$ss -codec copy public/download/ok$file_reup";
                                        }
                                    } else{
                                        if(strpos($duration, 'M')>0){
                                            ...
                                            if($catdau>60){
                                                $mt=floor($catdau/60);
                                                $st=$catdau-$mt*60;
                                                $cmd = "$ffmpeg -y -ss 00:$mt:$st -i public/download/$file_reup -to 00:$mm:$ss -codec copy public/download/ok$file_reup";
                                            } else{
                                                $cmd = "$ffmpeg -y -ss 00:00:$catdau -i public/download/$file_reup -to 00:$mm:$ss -codec copy public/download/ok$file_reup";
                                            }
                                        } else{
                                            $s = get_nb($duration,'T','S');
                                            $cmd = "$ffmpeg -y -ss 00:00:05 -i public/download/$file_reup -to 00:00:$s -codec copy public/download/ok$file_reup";
                                        }
                                    }

                                    $cmd1 = "$ffmpeg -y -i public/download/ok$file_reup -codec copy -metadata title='' -metadata artist='' -metadata album_artist='' -metadata album='' -metadata date='' -metadata track='' -metadata genre='' -metadata publisher='' -metadata encoded_by='' -metadata copyright='' -metadata composer='' -metadata performer='' -metadata TIT1='' -metadata TIT3='' -metadata disc='' -metadata TKEY='' -metadata TBPM='' -metadata language='eng' -metadata encoder='' -preset superfast public/download/upload$file_reup";
                                    shell_exec($cmd);
                                    shell_exec($cmd1);
                                ...
                                // *******end down va xu li video*******
                                    ...
                                    $videoPath = "public/download/upload$file_reup";
				    ...
				    $catdd=ghep($reupvideo_rows->tag,$reupchannel_rows->tag,$reupchannel_rows->tag_type,1);
				    ...
				    $tag=rtrim($l, ',');
                                    ...
                                    try{
                                        ...
                                        // xoa file 
                                        array_map('unlink', glob("public/download/*"));
                                        // end xoa file
                                        // If you want to make other calls after the file upload, set setDefer back to false
                                        $client->setDefer(false);
