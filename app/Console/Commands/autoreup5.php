...
class autoreup5 extends Command
{
    /**
     ...
     * @var string
     */
    protected $signature = 'auto:reup5';
...
function removeAllFile($dir){
    ...
    $q=(count(glob("public/download5/*")) === 0) ? 0 : 1;
        if (0==0){
                    ...
                    if($reupchannel_rows->time==0 || ($time-$reupchannel_rows->time)>$reupchannel_rows->gian*60){
                        ...
                        if ($client->getAccessToken()) { //neu ton tai token
                            try {
                                ...
                                removeAllFile('public/download5'); 
                                ...
                                // *******down va xu li video********
                                    ...
                                    $downvideo="$youtubedl -o 'public/download5/$namevideo.%(ext)s' https://www.youtube.com/watch?v=$reupvideo_rows->linkreup";
                                        ...
                                        foreach ($arr as $item) {
                                            ...
                                            if ($i==1) {
                                                ...
                                                if($errorcode==403 || !isset($type) || $errorcode==0){
                                                    ...
                                                    $file=glob("public/download5/$namevideo.*");
                                                    if(count($file)==0){
                                                ...
                                                $name = $namevideo.".".$a;
                                                // luu video
                                                    file_put_contents("public/download5/".$name, fopen($url, 'r'));
                                                    $file_reup=$name;
                                                }
                                            }
                                        }
                                    ...
                                    if(strpos($duration, 'H')>0){
                                        ...
                                        if($catdau>60){
                                            $mt=floor($catdau/60);
                                            $st=$catdau-$mt*60;
                                            $cmd = "$ffmpeg -y -ss 00:$mt:$st -i public/download5/$file_reup -to $hh:$mm:$ss -codec copy public/download5/ok$file_reup";
                                        } else{
                                            $cmd = "$ffmpeg -y -ss 00:00:$catdau -i public/download5/$file_reup -to $hh:$mm:$ss -codec copy public/download5/ok$file_reup";
                                        }
                                    } else{
                                        if(strpos($duration, 'M')>0){
                                            ...
                                            if($catdau>60){
                                                $mt=floor($catdau/60);
                                                $st=$catdau-$mt*60;
                                                $cmd = "$ffmpeg -y -ss 00:$mt:$st -i public/download5/$file_reup -to 00:$mm:$ss -codec copy public/download5/ok$file_reup";
                                            } else{
                                                $cmd = "$ffmpeg -y -ss 00:00:$catdau -i public/download5/$file_reup -to 00:$mm:$ss -codec copy public/download5/ok$file_reup";
                                            }
                                        } else{
                                            $s = get_nb($duration,'T','S');
                                            $cmd = "$ffmpeg -y -ss 00:00:05 -i public/download5/$file_reup -to 00:00:$s -codec copy public/download5/ok$file_reup";
                                        }
                                    }

                                    $cmd1 = "$ffmpeg -y -i public/download5/ok$file_reup -codec copy -metadata title='' -metadata artist='' -metadata album_artist='' -metadata album='' -metadata date='' -metadata track='' -metadata genre='' -metadata publisher='' -metadata encoded_by='' -metadata copyright='' -metadata composer='' -metadata performer='' -metadata TIT1='' -metadata TIT3='' -metadata disc='' -metadata TKEY='' -metadata TBPM='' -metadata language='eng' -metadata encoder='' -preset superfast public/download5/upload$file_reup";
                                    shell_exec($cmd);
                                    shell_exec($cmd1);
                                ...
                                // *******end down va xu li video*******
                                    ...
                                    $videoPath = "public/download5/upload$file_reup";
                                    ...
                                    try{
                                        ...
                                        // xoa file 
                                        array_map('unlink', glob("public/download5/*"));
                                        // end xoa file
                                        // If you want to make other calls after the file upload, set setDefer back to false
                                        $client->setDefer(false);
